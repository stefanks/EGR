...
\begin{align*}
 ...
 & = \mbox{tr}(\mbox{Var}^{S_k}(y_k))  \\
  & \leq 2\left(\frac{t_k}{(t_k+u_k)s_k}\right)^2 \sum_{j\in S_k}\mbox{tr}\left(\mbox{Var}^{S_k}(\nabla f^j(x_k)) + \mbox{Var}^{S_k}(\nabla f^j(\phi_{k-1}^j)) \right) \label{holdit}
\end{align*}
[using the fact that the variance of a sum of two random variables is less than or equal to twice the sum of their 
variances [since Var(X+Y)=2(Var(X)+Var(Y))-(Var(X-Y))<=2(Var(X)+Var(Y))]].
By Assumption A.4, the first term in the sum is bounded by $ v^2$, and for the second term we have
\begin{align*}
\mbox{tr}(\mbox{Var}^{S_k}(\nabla f^j(\phi_{k-1}^j)) ) &=\mbox{tr}(\mbox{Var}^{S_k}(\nabla f^j(\phi_{k-1}^j)-\nabla f^j(x_k) + \nabla f^j(x_k)) ) \\
                                          &\leq 2\mbox{tr}(( \mbox{Var}^{S_k}(\nabla f^j(\phi_{k-1}^j)-\nabla f^j(x_k) )) + \mbox{tr}(\mbox{Var}^{S_k}(\nabla f^j(x_k))) ) \\
                                                                               &\leq 2(\E^{S_k}[\| \nabla f^j(\phi_{k-1}^j)-\nabla f^j(x_k) )\|^2] + v^2 ) \\
                                           &\leq 2( L_j^2 \E^{S_k}[ \|\phi_{k-1}^j- x_k \|^2] +  v^ 2) 
\end{align*}
Now if we assume all the iterates are bounded than there is a constant $R>0$ such that $\|\phi_{k-1}^j)-\nabla f^j(x_k) \| \leq R$
for all $k$ and $j$.
Putting this together, (\ref{hold it}) becomes
\begin{align*}
 \mbox{tr}(\mbox{Var}^{S_k}(y_k)) & = 2\left(\frac{t_k}{(t_k+u_k)s_k}\right)^2   \sum_{j\in S_k} (3v^2+L_j^2 R^2 ) \right)\\
 & \leq 2\left(\frac{t_k}{t_k+u_k}\right)^2\frac{3v^2+L^2 R^2}{s_k}.
\end{align*}
\normalsize
Then, defining $\gamma_k := \displaystyle\sqrt{\frac{6v^2+2L^2 R^2}{s_k}}$,
\[
 E^{S_k}\left[\| y_k - \nabla F_{T_{k+1}}(x_k)\|\right] \leq \left(\frac{t_k}{t_k+u_k}\right)\sqrt{\frac{6v^2+2L^2 R^2}{s_k}} \leq \frac{1}{1+\eta}\gamma_k
\]
...
\noindent{\texttt{Only-add}}: algorithms that do not recompute gradients sampled previously. They include the stochastic gradient method $(u_k=1)$ and dynamic sampling methods, where $u_k$ typically grows as a function of $k$. Both methods are very successful in practice and the behavior of the sequence $\{F(x_k)\}$ is well understood.

\medskip\noindent{\texttt{Only-update}}: algorithms that do not sample new gradients. The classic gradient descent algorithm, SAG, and SAGA fall in this category. These algorithms work on a static batch of size $m$ and their convergence results apply to the sequence $\{ 
	\frac{1}{m} \sum_{i =1}^m f^i(x_k)\}$, defined in \eqref{saa}.

\medskip\noindent{\texttt{egr-lin}}. This version of the EGR method employs a constant number of replacements and a constant number of new samples at each iteration, which give rise to a linear growth in $t_k$.

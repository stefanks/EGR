...
\noindent{\texttt{Only-add}}: algorithms that do not recompute old gradients. They include the stochastic gradient method $(u_k=1)$ and dynamic sampling methods, where $u_k$ typically grows as a function of $k$. Both methods are very successful in practice and the behavior of the sequence $\{F(x_k)\}$ is well understood.

\medskip\noindent{\texttt{Only-update}}: algorithms that do not sample new gradients.The classic gradient descent algorithm, SAG, and SAGA fall in this category. These algorithms work on a static batch of size $m$ and their convergence results apply to the sequence $\{ F_m(x_k) \}$ defined in \eqref{saa}.
